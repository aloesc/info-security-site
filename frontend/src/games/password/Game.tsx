...
export default function PasswordGame() {
  ...
  const levelProgress: Record<StrengthLevel, number> = { weak: 1, medium: 2, strong: 3, unbreakable: 4 };

  useEffect(() => {
    if (gameOver) return;
    const currentProgress = levelProgress[strength.level];
    const requiredProgress = levelProgress[target.requiredLevel];
    if (currentProgress >= requiredProgress && password.length > 0) {
      if (level < MAX_LEVELS) {
